import 'package:flutter/material.dart';

class AppColors {
  AppColors._();

  // Monochrome Palette
  static const Color black = Color(0xFF000000);
  static const Color white = Color(0xFFFFFFFF);

  // Greys for depth and hierarchy
  static const Color darkGrey = Color(0xFF1E1E1E);
  static const Color mediumGrey = Color(0xFF757575);
  static const Color lightGrey = Color(0xFFE0E0E0);
  static const Color ultraLightGrey = Color(0xFFF5F5F5);

  // Semantic Aliases for Light Theme
  static const Color lightBackground = white;
  static const Color lightSurface = white; // Or ultraLightGrey
  static const Color lightTextPrimary = black;
  static const Color lightTextSecondary = mediumGrey;
  static const Color lightBorder = lightGrey;
  static const Color lightPrimary = black;
  static const Color lightOnPrimary = white;

  // Semantic Aliases for Dark Theme
  static const Color darkBackground = black;
  static const Color darkSurface = darkGrey; // Or black with border
  static const Color darkTextPrimary = white;
  static const Color darkTextSecondary = Color(
    0xFFAAAAAA,
  ); // Lighter grey for dark mode text
  static const Color darkBorder = Color(0xFF333333);
  static const Color darkPrimary = white;
  static const Color darkOnPrimary = black;
}
